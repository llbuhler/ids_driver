// ignore_for_file: file_names

class Clrs {
  static int blue = 0xFF034C8C;
  static int dkblue = 0xFF05325C;
  static int ltblue = 0xFFC9D8E5;
  static int dkgray = 0xFF606060;
  static int ltgray = 0xFF3E9E9E9;
  static int taco = 0xFFE8B962;
  static int laser = 0xFF1DF236;
  static int congo = 0XFF5D3D3D;
  static int white = 0xFFFFFFFF;
  static int red = 0xFFFF0000;
  static int transparent = 0x00FFFFFF;
  static int black = 0xFF000000;
  static int ltgreen = 0xFFC6E6C9;
  static int ltred = 0xffE6C6C9;
  static int grey = 0xFF9E9E9E;
  static int green = 0xFF00FF00;
  static int dkgreen = 0xFF439771;
  static int yellow = 0xFFFCFB00;
  static int ltyellow = 0xFFFFECB3;
}
